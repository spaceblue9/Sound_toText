...
## pyinstaller --onefile --icon=video.ico  --copy-metadata imageio --copy-metadata imageio-ffmpeg --hidden-import=imageio --hidden-import=imageio_ffmpeg --add-data "C:\Users\user\Desktop\Sound_toText\.venv\Lib\site-packages\whisper;whisper" transcribe_whisper.py 
...
def transcribe_chunk(
    ...
    """
    อ่านเสียงช่วง start-end จากไฟล์แล้วถอดเสียง
    (เวอร์ชันนี้เรียกแบบทีละ chunk ในลูปเดียว ไม่ใช้ multi-thread)
    """
    if end <= start:
        return start, ""

    from moviepy.audio.io.AudioFileClip import AudioFileClip  # import ในฟังก์ชันกันปัญหา multiprocessing บางแพลตฟอร์ม

    temp_chunk_path = f"chunk_{start}_{end}.mp3"
    audio_clip = AudioFileClip(audio_path)
    segment = audio_clip.subclipped(start, end)
    segment.write_audiofile(temp_chunk_path, codec="mp3", logger=None)
    audio_clip.close()
...
def transcribe_file(
    ...
    audio_clip = AudioFileClip(audio_path)
    duration_sec = int(audio_clip.duration)
    audio_clip.close()
    ...
    print(f"🚀 เริ่มถอดเสียงทีละ chunk (chunk_sec={chunk_sec})...")

    # เตรียมรายการ chunk และกันไม่ให้มีช่วง 0 วินาที
    chunks = []
    for start in range(0, duration_sec, chunk_sec):
    ...
    with tqdm(total=duration_sec, unit="sec", desc="📊 Progress") as pbar:
        for start, end in chunks:
            start_pos, text = transcribe_chunk(
                model=model,
            ...
            extracted[start_pos] = text
            pbar.update(end - start)
            pbar.set_postfix_str(f"{sec_to_time(end)} / {sec_to_time(duration_sec)}")

    final_text = " ".join(extracted[k] for k in sorted(extracted.keys())).strip()
...
def parse_args():
    desc_text = (
        ...
        "รองรับแบ่งไฟล์เป็น chunk ย่อย ๆ เพื่อควบคุมเวลาและหน่วยความจำ\n"
